...
def basic_als_recommender(filename, seed):
    ...
    *ALS.setSeed()*). The following parameters must be used in the ALS
    ...
    als = ALS(maxIter=5, rank=70, userCol="userId", itemCol="movieId", ratingCol="rating",
...
def global_average(filename, seed):
    ...
    ratings = spark.createDataFrame(ratingsRDD)
    (training, test) = ratings.randomSplit([0.8, 0.2], seed=seed)
    average = training.agg({"rating": "avg"}).first()[0]
    return average
...
def global_average_recommender(filename, seed):
    ...
    (training, test) = ratings.randomSplit([0.8, 0.2], seed=seed)
    average = training.agg({"rating": "avg"}).first()[0]
    test = test.withColumn("prediction", lit(average))
    evaluator = RegressionEvaluator(metricName="rmse", labelCol="rating",
                                    predictionCol="prediction")
...
def means_and_interaction(filename, seed, n):
    ...
    training = training.withColumn("user_item_interaction",
                                   training.rating - (training.user_mean + training.item_mean
                                                      - global_mean))
    training = training.orderBy("userId", "movieId").limit(n)
    return [training.collect()[i] for i in range(n)]
